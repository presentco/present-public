 ...
 */
public class LiveServerAcceptanceTest {

  private static final int PORT = 8888;

  @Test public void testLocal() throws Exception {
    test("local.present.co");
  }

  @Test public void testStaging() throws Exception {
    test("live.staging.present.co");
  }

  @Test public void testProduction() throws Exception {
    test("live.present.co");
    test("live.test.bubble.network");
  }

  private void test(String host) throws Exception {
    for (int i = 0; i < 2; i++) {
      ...
      UserResponse author =
          new UserResponse("id", "Bob Lee", "Bob", "https://goo.gl/E08v0o", "",
              new ArrayList<>(), new ArrayList<>(), null, "https://present.co", true);
      CommentResponse comment = new CommentResponse.Builder()
          ...
          .index(0)
          .build();
      LiveService bs = InternalLiveClient.connectTo("https://" + host + ":" + PORT + "/api");
      ...
      RequestHeader header =
          new RequestHeader(UUID.randomUUID().toString(), UUID.randomUUID().toString(), "IGNORED",
              Platform.TEST, 1, "1", "1", null, null, null, "women-only");

      String userId = "1234";
      LiveClient client = new LiveClient(host, PORT, header, userId, groupId, listener);

      waitFor(listener.readyLatch);

      bs.dispatchComment(new DispatchCommentRequest(clientId, groupId,
          ByteString.of(CommentResponse.ADAPTER.encode(comment)), null));
  ...
  private class CommentListener implements LiveClient.Listener {
    ...
    @Override public void deleted(CommentResponse comment) {}
