//
//  AddMembersFromPresentFriendsViewController.swift
//  Present
//
...
public class AddMembersFromPresentFriendsViewController: AddMembersFromPersonDataViewController, AddMemberListCellSelection
{
    var presentFriendsSection: TableViewModel.Section<AddMemberListCell>?
    ...
    override func initTable()
    {
        ...
        emptyView.do {
            $0.emptyText = "You haven't connected with friends on Present yet. Add them to join the conversation!"
            $0.button.addTarget(self, action: #selector(launchAddFriends), for: .touchUpInside)
        }
        
        self.presentFriendsSection = tableViewModel.addSection(
    ...
    override func fetchData()
    {
        ...
        personManager.myFriends.observable()
            .onNext {
                self.endActivity()
                self.presentFriendsSection?.items = $0
