...
public class PresentTabs: ApplicationServices
{
    public class func configurePresentTabs(bar: TabmanBar.Config, noTabs: Bool = false)
    {
        ...
        bar.appearance = TabmanBar.Appearance {
            ...
            $0.layout.edgeInset = 8.0
            
            // Specify if tabs are centered or left aligned within the tab bar.
    ...
    // Badge count is currently capped at 99
    // TODO: Ideally this would show "99+" but we'll have to inset the badge more (doesn't fit).
    public class func badgedItem(title: String, badgeCount: Int) -> TabmanBar.Item {
        return badgeCount > 0 ?
            TabmanBar.Item(title: title, image: PresentTabs.renderBadgeForTab(count: min(99,badgeCount))) :
            TabmanBar.Item(title: title)
    }
    
    private static let badgeVersion = 1
    
    /// Render the badge to be placed to the left of the tab text.
    // param: title currently just for logging
    public class func renderBadgeForTab(count: Int)->UIImage
    {
        let cacheKey = "badge:version:\(badgeVersion):count\(count)"
        if let image = applicationServices.imageManager.getStoredImage(forCacheKey: cacheKey) {
            //log("returning cached badge for: \(count)")
            return image
        } else {
            //log("rendering badge for: \(count)")
        }
        ...
        // Note: We were planning to render the full text and badge button here but TabMan seems to insist that
        // Note: the image be square. So we only render the badge portion and use the Item's built in left image.
        ...
        applicationServices.imageManager.cacheImage(image, forCacheKey: cacheKey)
        return image
    }
