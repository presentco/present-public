...
{
    typealias MessageRange = [MessageModel]
    /**
     ...
     */
    typealias MessagePredicate = ( _ message:MessageModel, _ index:Int, _ range:MessageRange )->(Bool)
    
    let groupMessagesGapTime = 10 * MSEC_PER_MIN
    
    func annotateMessages( _ messages : [MessageModel] )
    {
     ...
     */
    func partitionByPredicate( _ messages : [MessageModel], predicate : MessagePredicate ) -> [MessageRange]
    {
